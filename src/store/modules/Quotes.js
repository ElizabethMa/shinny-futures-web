  ...
  getters: {
    ...
    GET_QUOTES_BY_INPUT: (state) => (str) => {
      ...
      for (let i in state.SymbolsList) {
        ...
        if (symbol.instrument_id.toLowerCase().includes(str) || symbol.ins_name.toLowerCase().includes(str)) result.push({value: symbol.instrument_id})
      }
      if (result.length > 0) return result
      for (let i in state.SymbolsList) {
        let symbol = state[state.SymbolsList[i]]
        if (symbol.py.includes(str)) result.push({value: symbol.instrument_id})
      }
      return result
    }
  }
